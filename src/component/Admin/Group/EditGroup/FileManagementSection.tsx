...
const FileManagementSection = () => {
  ...
  const onAllowRelocateChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      setGroup((p: GroupEnt) => ({
        ...p,
  ...
  const onAllowedNodeChange = useCallback(
    (e: number[]) => {
      setGroup((p: GroupEnt) => ({ ...p, settings: { ...p.settings, available_nodes: e?.length ? e : undefined } }));
    },
    [setGroup],
  );
  ...
  const onAllowSelectNodeChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      setGroup((p: GroupEnt) => ({
        ...p,
        permissions: new Boolset(p.permissions).set(GroupPermission.select_node, e.target.checked).toString(),
      }));
    },
    [setGroup],
  );
          ...
          <>
            ...
            <SettingForm lgWidth={5}>
              <FormControl fullWidth>
                <FormControlLabel
                  control={
                    <Switch checked={permission.enabled(GroupPermission.relocate)} onChange={onAllowRelocateChange} />
                  }
            ...
            <SettingForm title={t("group.allowedNodes")} lgWidth={5}>
              <FormControl fullWidth>
                <MultipleNodeSelectionInput
                  value={values.settings?.available_nodes ?? []}
                  onChange={onAllowedNodeChange}
                />
            ...
            <SettingForm lgWidth={5}>
              <FormControl fullWidth>
                <FormControlLabel
                  control={
                    <Switch
                      checked={permission.enabled(GroupPermission.select_node)}
