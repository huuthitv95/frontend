...
import { preparePermissionSetting } from "../../../FileManager/Dialogs/SetPermission";
import { useDefaultPermissionSetting } from "../../../FileManager/Dialogs/Share/ShareDialog";
import FileTypeIcon from "../../../FileManager/Explorer/FileTypeIcon";
import PermissionList, { PermissionListItem } from "../../../FileManager/FileInfo/Permission/PermissionList";
...
const ShareForm = ({ values }: { values: Share }) => {
  ...
  const [listItem, setListItem] = useState<PermissionListItem[] | undefined>(undefined);
  ...
  useEffect(() => {
    if (values.permission_setting) {
      preparePermissionSetting(values.permission_setting, t, defaultPermissionSetting, dispatch).then((res) => {
        setListItem(res);
      });
    }
  }, [values.permission_setting, defaultPermissionSetting]);
    ...
    <>
      ...
      <Box>
        <Grid container spacing={isMobile ? 2 : 3} alignItems={"stretch"}>
          ...
          <SettingForm title={t("share.price")} noContainer lgWidth={2}>
            <Typography variant={"body2"} color={"textSecondary"}>
              {values.price ?? 0}
            </Typography>
          </SettingForm>
