...
import { FileType, PolicyType } from "../../../../api/explorer";
...
const FileForm = () => {
  ...
  const onPrimaryStoragePolicyChange = useCallback(
    (value: number) => {
      setFile((prev) => ({ ...prev, storage_policy_files: value }));
    },
    [setFile],
  );
        ...
        <Grid container spacing={isMobile ? 2 : 3} alignItems={"stretch"}>
          ...
          <SettingForm title={t("file.primaryStoragePolicy")} noContainer lgWidth={3}>
            <SinglePolicySelectionInput
              filter={(policy) => policy.type !== PolicyType.load_balance}
              simplified
              value={values.storage_policy_files ?? 0}
              onChange={onPrimaryStoragePolicyChange}
            />
          </SettingForm>
