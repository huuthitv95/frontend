...
const UserForm = ({ reload, setLoading }: { reload: () => void; setLoading: (loading: boolean) => void }) => {
  ...
  const onOriginGroupChange = useCallback(
    (value: string) => {
      setUser((prev) => ({ ...prev, previous_group: value == " " ? undefined : parseInt(value) ?? undefined }));
    },
    [setUser],
  );
  ...
  const onGroupExpiredChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      setUser((prev) => ({ ...prev, group_expires: e.target.value ? e.target.value : undefined }));
    },
    [setUser],
  );
  ...
  const onCreditChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      setUser((prev) => ({ ...prev, credit: parseInt(e.target.value) }));
    },
    [setUser],
  );
        ...
        <Box sx={{ flexGrow: 1 }}>
          <Grid container spacing={isMobile ? 2 : 3}>
            ...
            <SettingForm title={t("user.originUserGroup")} noContainer lgWidth={6}>
              <GroupSelectionInput
                value={values.previous_group?.toString() ?? " "}
            ...
            <SettingForm title={t("user.groupExpired")} noContainer lgWidth={6}>
              <DenseFilledTextField fullWidth value={values.group_expires ?? ""} onChange={onGroupExpiredChange} />
