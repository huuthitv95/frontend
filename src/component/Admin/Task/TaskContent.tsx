...
import { StoragePolicy, Task } from "../../../api/dashboard";
...
export interface TaskContentProps {
  task: Task;
  allStoragePolicies?: StoragePolicy[];
...
export const TaskContent = memo(({ task, allStoragePolicies, openEntity, openFile }: TaskContentProps) => {
  ...
  if (userTaskTypes.includes(task.type ?? "")) {
    ...
    return (
      <Typography variant="body2">
        <TaskSummaryTitle
          storagePolicies={allStoragePolicies?.map((p) => ({ ...p, max_size: p.max_size ?? 0, id: p.hash_id ?? "" }))}
