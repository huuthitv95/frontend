...
import { batchDeleteTasks, getStoragePolicyList, getTaskList } from "../../../api/api";
import { AdminListService, ListStoragePolicyResponse, StoragePolicy, Task } from "../../../api/dashboard";
import { useAppDispatch } from "../../../redux/hooks";
import { confirmOperation } from "../../../redux/thunks/dialog";
import { refreshPolicyOptions } from "../../../redux/thunks/settings";
...
const TaskList = () => {
  ...
  const [allStoragePolicies, setAllStoragePolicies] = useState<StoragePolicy[]>([]);
  ...
  useEffect(() => {
    dispatch(getStoragePolicyList({ page: 1, page_size: 1000, order_by: "id", order_direction: "asc" })).then(
      (res: ListStoragePolicyResponse) => {
        setAllStoragePolicies(res.policies);
      },
    );
  }, []);
          ...
          <Table size="small" stickyHeader sx={{ width: "100%", tableLayout: "fixed" }}>
            ...
            <TableBody>
              {!loading &&
                tasks.map((task) => (
                  <TaskRow
                    allStoragePolicies={allStoragePolicies}
