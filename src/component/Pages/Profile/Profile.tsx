...
import { openReportAbusedUserDialog } from "../../../redux/thunks/dialog.ts";
...
import { DenseSelect, SecondaryButton } from "../../Common/StyledComponents.tsx";
...
const Profile = () => {
  ...
  const showReportAbuse = useMemo(() => {
    return SessionManager.currentLoginOrNull()?.user?.id !== user?.id;
  }, [user]);
      ...
      <Container maxWidth="lg">
        <PageHeader
          ...
          secondaryAction={
            showReportAbuse ? (
              <SecondaryButton
                onClick={() => dispatch(openReportAbusedUserDialog(user))}
                startIcon={isMobile ? undefined : <WarningOutlined />}
                variant="contained"
              >
