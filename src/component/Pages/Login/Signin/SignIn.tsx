...
import PhaseSelectSession from "../Phases/PhaseSelectSession.tsx";
...
const EmailLogin = ({ oauthConsent }: SignInProps) => {
  ...
  const [useSessionSelection, setUseSessionSelection] = useState(false);
  ...
  useEffect(() => {
    const init = async () => {
      if (isOAuthFlow) {
        // Store current URL for OpenID redirect
        localStorage.setItem(OAUTH_REDIRECT_KEY, window.location.href);
      ...
      if (query.get("phase") == "email") {
        return;
      }
      const sessions = SessionManager.validSession();
      if (sessions.length > 0) {
        setPhase(EmailLoginPhase.SelectSession);
        setUseSessionSelection(true);
      }
    };
    init();
  ...
  const phaseConfig = useMemo((): phaseSetting => {
    var phaseSetting: phaseSetting = {
      ...
      previous: EmailLoginPhase.SelectSession,
    };
    switch (phase) {
      case EmailLoginPhase.CollectEmail:
        phaseSetting = {
          title: t("login.siginToYourAccount"),
          nextButtonText: t("login.continue"),
          showBackButton: useSessionSelection,
          previous: EmailLoginPhase.SelectSession,
        };
        break;
      case EmailLoginPhase.SignupNeeded:
      ...
      case EmailLoginPhase.CollectPassword:
        ...
        break;
      case EmailLoginPhase.SelectSession:
        phaseSetting = {
          title: t("login.selectAccountToUse"),
          nextButtonText: t("login.signIn"),
          showBackButton: false,
        };
        break;
    ...
    return phaseSetting;
  }, [phase, t, loading, useSessionSelection]);
        ...
        <SwitchTransition>
          ...
          >
            <Box>
              ...
              {phase === EmailLoginPhase.SelectSession && (
                <PhaseSelectSession
                  onLoginWithOtherAccount={(email?: string) => {
                    setPhase(EmailLoginPhase.CollectEmail);
                    if (email) {
                      setEmail(email);
                    }
                  }}
                  redirect={isOAuthFlow ? undefined : query.get("redirect")}
                  onSessionSwitch={isOAuthFlow ? handleOAuthSessionSwitch : undefined}
                />
              )}
