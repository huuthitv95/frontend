...
const PreferenceSetting = ({ setting, setSetting }: PreferenceSettingProps) => {
  ...
  const [notifyStoragePolicyChange, setNotifyStoragePolicyChange] = useState(
    SessionManager.getWithFallback(UserSettings.NotifyStoragePolicyChange),
  );
  ...
  const onNotifyStoragePolicyChangeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setNotifyStoragePolicyChange(e.target.checked);
    SessionManager.set(UserSettings.NotifyStoragePolicyChange, e.target.checked);
  };
  ...
  return (
    <Stack spacing={3}>
      ...
      <SettingForm title={t("fileManager.storagePolicy")} lgWidth={12}>
        <SmallFormControlLabel
          control={
            <Checkbox size="small" checked={notifyStoragePolicyChange} onChange={onNotifyStoragePolicyChangeChange} />
          }
          label={t("application:setting.notifyStoragePolicyChange")}
        />
        <FormHelperText>{t("application:setting.notifyStoragePolicyChangeDes")}</FormHelperText>
      </SettingForm>
    </Stack>
  );
};
