...
const TaskSummaryTitle = ({ type, summary, storagePolicies, isInDashboard = false }: TaskSummaryTitleProps) => {
  ...
  const policyOptonToName = storagePolicies ? storagePolicies : policyOption;
        ...
        <>
          {
            <Trans
              ...
              values={{
                more: (summary?.props.src_multiple?.length ?? 0) > 3 ? "..." : "",
                policy: policyOptonToName
                  ? policyOptonToName.find((p) => p.id == summary?.props.dst_policy_id)?.name ?? "Unknown"
                  : "",
              }}
            />
          }
