...
const BasicInfo = ({ target }: BasicInfoProps) => {
  ...
  const storagePolicy = useMemo(() => {
    if (target.extended_info) {
      ...
      if (target.extended_info.storage_policy_inherited) {
        return target.extended_info.storage_policy.name + " " + t("fileManager.inheritedFromParent");
      }
  ...
  return (
    <>
      ...
      <InfoRow title={t("application:fileManager.storagePolicy")} content={storagePolicy} />
      <InfoRow
        title={t("application:fileManager.myPermissions")}
        content={
          target.owned ? t(isSymbolicLink ? "fileManager.ownedSymbolic" : "fileManager.owned") : permissionSummary
        }
      />
