...
import { Entity, EntityType, FilePermission, FileResponse } from "../../../api/explorer.ts";
...
const VersionControl = () => {
  ...
  const [canUpdate, canDelete] = useMemo(() => {
    if (target?.permission) {
      ...
      return [bs.enabled(FilePermission.update), bs.enabled(FilePermission.delete)] as const;
    }
  ...
  return (
    <>
      ...
      >
        ...
        {canUpdate && actionTarget?.id !== fileExtended?.primary_entity && (
          <SquareMenuItem onClick={setAsCurrent} dense>
            <ListItemText>{t("application:fileManager.setAsCurrent")}</ListItemText>
          </SquareMenuItem>
        )}
        {canDelete && actionTarget?.id !== fileExtended?.primary_entity && (
