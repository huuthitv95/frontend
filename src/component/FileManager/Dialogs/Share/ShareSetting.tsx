...
const ShareSettingContent = ({ setting, file, editing, onSettingChange }: ShareSettingProps) => {
  ...
  const pointEnabled = useAppSelector((state) => state.siteConfig.vas.config.point_enabled);
  const shareGainRate = useAppSelector((state) => state.siteConfig.vas.config.share_point_gain_rate);
  const vasLoaded = useAppSelector((state) => state.siteConfig.vas.loaded);

  useEffect(() => {
    if (vasLoaded != ConfigLoadState.Loaded) {
  ...
  const handleChange = (prop: "expires" | "downloads" | "price") => (event: { target: { value: string } }) => {
    onSettingChange({
      ...setting,
      [prop + "_val"]: parseInt(event.target.value),
    });
  };

  const handleCheck = (prop: "is_private" | "share_view" | "show_readme" | "expires" | "downloads" | "price") => () => {
    if (!setting[prop]) {
        ...
        <Accordion expanded={expanded === "price"} onChange={handleExpand("price")}>
          <AccordionSummary aria-controls="panel1a-content" id="panel1a-header">
            <StyledListItemButton>
              ...
              <ListItemText primary={t("application:vas.payToDownload")} />
              <ListItemSecondaryAction>
                <Checkbox checked={setting.price} onChange={handleCheck("price")} />
              </ListItemSecondaryAction>
            </StyledListItemButton>
          </AccordionSummary>
          <AccordionDetails>
            <FilledTextField
              ...
              type="number"
              inputProps={{ min: 0 }}
              value={setting.price_val}
              onChange={handleChange("price")}
              label={t("application:vas.creditToBePaid")}
            />
            {setting.price_val !== 0 && shareGainRate && shareGainRate != 100 && (
              <Typography sx={{ mt: 1 }} variant="body2">
                {t("application:vas.creditGainPredict", {
                  num: Math.ceil((setting.price_val * shareGainRate) / 100),
                })}
              </Typography>
            )}
          </AccordionDetails>
