...
const isFolderLink = (link: string) => link.endsWith("/");
  ...
  const textContents = useMemo(() => {
    ...
    return targets
      .map((target) => {
        ...
        const suffix = isFolderLink(target.link) ? `<${relativePathPlaceholder}>` : "";
        if (!showFileName) return `${finalLink}${suffix}`;
        return `[${getDisplayName(target.file_url)}] ${finalLink}${suffix}`;
      })
      .join("\n");
      ...
      <Stack spacing={0.5}>
        {targets?.map((target, index) => (
          <DirectLinkItem
            ...
            index={index}
            isFolder={isFolderLink(target.link)}
