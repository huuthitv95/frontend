...
import { FilePermission, FileType, Metadata } from "../../../../api/explorer.ts";
...
export const LockedIcon = ({ isList }: { isList?: boolean }) => {
  const { t } = useTranslation();
  return (
    <Tooltip enterDelay={200} title={t("application:fileManager.youDontHaveReadPermissionToThisFile")}>
      <Box
        sx={{
          mt: isList ? "5px" : 0,
          padding: isList ? 0 : "14px 12px 14px 0",
        }}
      >
        <LockClosedOutlined fontSize={"small"} color={"action"} />
      </Box>
    </Tooltip>
  );
};
...
export const useFileBlockState = (props: FileBlockProps) => {
  ...
  const showLock = useMemo(() => {
    if (file.permission) {
      return !new Boolset(file.permission).enabled(FilePermission.read);
    }
