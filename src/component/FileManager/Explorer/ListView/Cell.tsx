...
import { CustomProps, FilePermission, FileType, Metadata } from "../../../../api/explorer.ts";
...
import { LockedIcon, ThumbLoadingPlaceholder, ThumbPopoverImg } from "../GridView/GridFile.tsx";
...
const FileNameCell = memo((props: CellProps) => {
  ...
  const { file, uploading, noThumb, showLock, fileTag, search, isSelected, showThumb, thumbWidth, thumbHeight } = props;
      ...
      <>
        ...
        >
          <Box {...(noThumb || isMobile || isTouch ? {} : hoverState)} sx={{ display: "flex", alignItems: "center" }}>
            <ListThumbnail file={file} isSelected={isSelected} noThumb={noThumb} showLock={showLock} />
          </Box>
          <Box
            sx={{
              display: "flex",
          ...
          >
            <Box sx={{ display: "flex", alignItems: "center", gap: 1, minWidth: 0 }}>
              ...
              {!isEditing && showLock && <LockedIcon isList />}
            </Box>
            <Box
              sx={{
                display: "flex",
...
interface MyPermissionCellProps {
  file: FmFile;
}

const MyPermissionCell = memo(({ file }: MyPermissionCellProps) => {
  const { t } = useTranslation();
  const permission = useMemo(() => {
    ...
    if (file.permission) {
      const bs = new Boolset(file.permission);
  ...
  const isSymbolicLink = file.metadata?.[Metadata.share_redirect];
  if (isSymbolicLink && !file.owned) {
    // Symbolic link under other's share
    return <Box />;
  }
  return (
    <NoWrapBox>
      {file.owned ? t(isSymbolicLink ? "fileManager.ownedSymbolic" : "fileManager.owned") : permissionSummary}
    </NoWrapBox>
  );
});
...
const Cell = memo((props: CellProps) => {
  ...
  switch (column.type) {
    ...
    case ColumType.permission:
      return <MyPermissionCell file={file} />;
