...
const CustomContent = React.memo(
  React.forwardRef(function CustomContent(props: CustomContentProps, ref) {
    ...
    const onMouseEnter = useCallback(() => {
      if (props.pinned && !props.pinnedReadonly) setShowDelete(true);
    }, [setShowDelete, props.pinned, props.pinnedReadonly]);

    const onMouseLeave = useCallback(() => {
      if (props.pinned && !props.pinnedReadonly) setShowDelete(false);
    }, [setShowDelete, props.pinned, props.pinnedReadonly]);
    ...
    return (
      ...
      >
        ...
        {FileItemIcon}
        {fileName}
        {!props.pinnedReadonly && <UnpinButton show={showDelete} uri={uri} />}
      </CustomContentRoot>
    );
  }),
);
...
const TreeFile = React.memo(
  React.forwardRef(function CustomTreeItem(props: TreeFileProps, ref: React.Ref<HTMLLIElement>) {
    const contentProps = useMemo(() => {
      const { level, file, notLoaded, fileIcon, loading, pinned, pinnedReadonly, canDrop } = props;
      return { level, file, notLoaded, fileIcon, loading, pinned, pinnedReadonly, canDrop };
    }, [
      props.level,
      props.file,
      props.notLoaded,
