...
import { openReportAbusedShareialog } from "../../../redux/thunks/dialog.ts";
...
const MoreActionMenu = ({ onClose, ...rest }: MenuProps) => {
  ...
  const onReportAbuseClicked = useCallback(() => {
    dispatch(openReportAbusedShareialog(fmIndex));
    onClose && onClose({}, "escapeKeyDown");
  }, [dispatch, onClose, fmIndex]);
  ...
  return (
    ...
    >
      ...
      {fs == Filesystem.share && (
        <SquareMenuItem onClick={onReportAbuseClicked}>
          <ListItemIcon>
      ...
      {(isLogin || fs == Filesystem.share) && <DenseDivider />}
