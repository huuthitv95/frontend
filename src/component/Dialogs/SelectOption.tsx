...
import { DialogContent, DialogContentText, List, ListItemButton, ListItemText } from "@mui/material";
...
const SelectOption = () => {
  ...
  const description = useAppSelector((state) => state.globalState.selectOptionDescription);
  ...
  return (
    ...
    >
      <DialogContent>
        {description && <DialogContentText sx={{ mb: 1 }}>{t(description)}</DialogContentText>}
        <List component="nav">
          {options?.map((o) => (
