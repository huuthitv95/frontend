...
export default class UploadManager {
  ...
  private getPolicy = (): StoragePolicy => {
    if (this.policy?.type !== PolicyType.load_balance) {
      return this.policy as StoragePolicy;
    }

    if (!this.policy.children || this.policy.children.length === 0) {
      const msg = "Load balance policy does not have any children";
      this.logger.error(msg, this.policy);
      throw new UploaderError(UploaderErrorName.NoPolicySelected, msg);
    }

    const availablePolicies = this.policy.children.filter((p) => (p.weight ?? 0) > 0);
    if (availablePolicies.length === 0) {
      const msg = "No available policies with weight > 0 in load balance policy";
      this.logger.error(msg, this.policy);
      throw new UploaderError(UploaderErrorName.NoPolicySelected, msg);
    }

    // Calculate total weight
    const totalWeight = availablePolicies.reduce((total, p) => total + (p.weight as number), 0);

    // Generate random number between 0 and totalWeight
    const randomValue = Math.random() * totalWeight;

    // Select policy based on weighted random
    let currentWeight = 0;
    for (const policy of availablePolicies) {
      currentWeight += policy.weight as number;
