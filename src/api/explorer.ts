...
export interface PermissionSettingReq {
  same_group?: string;
  everyone?: string;
  other?: string;
  anonymous?: string;
...
export interface FileActivitiesService {
  uri: string;
  page_size?: number;
  order_by?: string;
  order_direction?: string;
...
export const FileActivityFilter = {
  all: "",
  updates: "updates",
  reads: "reads",
  my: "my",
...
export interface SetPermissionService extends MultipleUriService {
  setting: PermissionSettingReq;
}
...
export interface MountPolicyService {
  uri: string;
  policy_id: string;
}
