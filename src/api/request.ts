...
export class AppError extends Error {
  ...
  constructor(resp: Response<any>) {
    ...
    } else if (i18n.exists(`vasErrors.${code}`, { ns: "common" })) {
      this.message = i18n.t(`vasErrors.${code}`, {
        ns: "common",
        message,
      });
...
export const CrHeaders = {
  ...
  purchase_ticket: CrHeaderPrefix + "Purchase-Ticket",
};
...
export function send<T = any>(
  ...
  return async (dispatch, _getState) => {
    try {
      ...
      if (opts.withPurchaseTicket) {
        const ticket = SessionManager.get(UserSettings.PurchaseTicket);
        if (ticket) {
          if (!axiosConf.headers) {
            axiosConf.headers = {};
          }
          axiosConf.headers[CrHeaders.purchase_ticket] = ticket;
        }
      }
