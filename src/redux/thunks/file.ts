...
import { decodeTimeFlowString, fileExtension, getFileLinkedUri } from "../../util";
...
export function loadFileThumb(index: number, file: FileResponse): AppThunk<Promise<string | null>> {
  return async (dispatch, getState) => {
    ...
    try {
      ...
      thumb.url = decodeTimeFlowString(thumb.url);
...
export function selectFile(index: number, file: FileResponse, e?: React.MouseEvent<HTMLElement>): AppThunk {
  return async (dispatch, getState) => {
    ...
    if (sidebarOpen) {
      const readable = !file.permission || new Boolset(file.permission).enabled(FilePermission.read);
      const cap = new Boolset(file.capability);
      if (canShowInfo(readable, cap)) {
...
export function mountStoragePolicy(index: number, policy: StoragePolicy): AppThunk {
  return async (dispatch, getState): Promise<void> => {
    const uri = getState().fileManager[index].pure_path;
    if (!uri) {
      return Promise.reject("No path to create file");
    }
    ...
    try {
      await longRunningTaskWithSnackbar(
        dispatch(sendMountStoragePolicy({ uri, policy_id: policy.id })),
        "application:modals.processingRestoring",
      );
    } catch (e) {
      success = false;
    ...
    if (success) {
      enqueueSnackbar({
        message: i18next.t("application:vas.folderPolicySwitched", {
          name: policy.name,
        }),
        variant: "success",
      });
...
const DirectLinkFolderMode = {
  walk_descendants: "walk_descendants",
...
export function batchGetDirectLinks(index: number, files: FileResponse[]): AppThunk {
  return async (dispatch, _getState) => {
    ...
    const hasFolders = files.some((f) => f.type === FileType.folder);
    const permission = SessionManager.currentUserGroupPermission();
    const hasFolderDirectLink = permission.enabled(GroupPermission.folder_direct_link);

    let folderMode = DirectLinkFolderMode.walk_descendants;
    if (hasFolders && hasFolderDirectLink) {
      try {
        folderMode = await dispatch(
          selectOption(
            [
              {
                name: i18next.t("modals.directLinkWalkDescendants"),
                description: i18next.t("modals.directLinkWalkDescendantsDesc"),
                value: DirectLinkFolderMode.walk_descendants,
              },
              {
                name: i18next.t("modals.directLinkFolderUrl"),
                description: i18next.t("modals.directLinkFolderUrlDesc"),
                value: DirectLinkFolderMode.folder_url,
              },
            ],
            "modals.directLinkFolderModeTitle",
            "modals.directLinkFolderModeDescription",
          ),
        );
      } catch (_e) {
        return;
    ...
    const res = await longRunningTaskWithSnackbar(
      dispatch(startBatchGetDirectLinks(files, folderMode === DirectLinkFolderMode.folder_url)),
...
function startBatchGetDirectLinks(files: FileResponse[], folderUrl?: boolean): AppThunk<Promise<DirectLink[]>> {
  return async (dispatch, _getState): Promise<DirectLink[]> => {
    ...
    if (folderUrl) {
      // Generate direct links for parent folders directly (no walking)
      const uris = files.map((f) => getFileLinkedUri(f));
      ...
      return await dispatch(getFileDirectLinks({ uris }));
    }
