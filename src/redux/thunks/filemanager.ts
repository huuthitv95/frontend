...
let lastStoragePolicy = "";
export function alertStoragePolicyChange(index: number): AppThunk {
  return async (_, getState) => {
    if (!SessionManager.getWithFallback(UserSettings.NotifyStoragePolicyChange)) {
      return;
    }
    const { storage_policy, single_file_view } = getState().fileManager[index]?.list ?? {};
    if (storage_policy && lastStoragePolicy != storage_policy.id && !single_file_view) {
      lastStoragePolicy = storage_policy.id;
      enqueueSnackbar({
        message: i18next.t("fileManager.currentStoragePolicy", { policy: storage_policy.name }),
        variant: "default",
