...
import {
  PermissionListItem,
  PermissionListToParams,
} from "../../component/FileManager/FileInfo/Permission/PermissionList.tsx";
...
export function createOrUpdateShareLink(
  index: number,
  file: FileResponse,
  permList: PermissionListItem[],
  setting: ShareSetting,
  existed?: string,
): AppThunk<Promise<string>> {
  return async (dispatch, getState) => {
    const req: ShareCreateService = {
      permissions: PermissionListToParams(permList),
      uri: file.path,
      ...
      price: setting.price && setting.price_val > 0 ? setting.price_val : undefined,
    };
