...
export function openReportAbusedShareialog(index: number): AppThunk<Promise<void>> {
  return async (dispatch, getState) => {
    ...
    const shareCrUri = new CrUri(reportPath);
...
export function openReportAbusedUserDialog(user: User): AppThunk<Promise<void>> {
  return async (dispatch, getState) => {
    dispatch(
      setReportAbuseDialog({
        open: true,
